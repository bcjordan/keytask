...
var firstPresses = ""; // First keypress in a pair
// var words = ["cat", "dog", "orange", "house"]

function newPrompt(){
	// Immediately generate, display, and log new prompt
	// For this task, display two letters. currChars is ['A', 'B'], for example
	currChars = words[Math.floor(Math.random()*words.length)].toUpperCase();
	currStart      = +new Date();
	
	firstPresses 		 = "";
	
	$('#prompt').text(currChars);
	
	console.log(currChars);
	$('#log').prepend('<div class="alert-message log-entry ">' + currStart + ", prompt, " + currChars + "</div>");
}
...
$('#input').keypress(
    function(e) {
				...
				var correct;
				var noPrompt = false;
				firstPresses = firstPresses + pressed;
				
				// Evaluate correctness
				if(firstPresses.length > 0 && firstPresses.length === currChars.length){
					correct = firstPresses === currChars;
					var correct_chart = correct ? 0 : 1			
					var delay = e.timeStamp - currStart;
					
					if(charting){
						chart.series[correct_chart].addPoint({x: (currStart - loadTime), y: delay, name: currChars + " -> " + firstPresses})
					}					
				...
				if(currChars){
					keyPresses.push({key: pressed, prompt: currChars, correct: correct,
				...
				if(firstPresses.length === 0 || !noPrompt){
					newPrompt();
				}
    }
);
